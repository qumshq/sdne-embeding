...
def MakeCoderSuro(EmbDim, NetName, Trained, NetNum, SurroNum):
    root = 'for_emb_10dim'
    path = f'{root}/Net/'
    pathEmbed = 'for_emb_10dim/Embed/'  # Embedding info path
    ...
    if Trained == 0:
        ...
        reTrain = 0.2
        G_make = makeOtherNets(G, path, NetName, NetNum)# 创建NetName个Graph对象
        ...
        TotalExtend = 0
        for i in range(len(G_make)):
            if random.random() <= reTrain or i == 0:# 疑问：如果不使用G_make中的其他图进行ExtendTrain则会导致其中一个维度的编码全部为0
                ExtendTrain(pathEmbed, NetName, TotalExtend, G_make[i])
                TotalExtend += 1
        path_Surrogate = f'{root}/Surrogate/'
        if os.path.isdir(path_Surrogate) == False:
            os.mkdir(path_Surrogate)
        Coder = makeEmbeding(pathEmbed, path, path_Surrogate, NetName, NetNum)
        ...
        for i in range(len(G_make)):
            if random.random() <= reTrain or i == 0:# 疑问：如果不使用G_make中的其他图进行ExtendTrain则会导致其中一个维度的编码全部为0
                ExtendTrain(pathEmbed, NetName, TotalExtend+1, G_make[i])
                TotalExtend += 1
