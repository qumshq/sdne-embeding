...
def makeEmbeding(pathEmbed, pathNet, path_Surrogate, NetName, NetNum):
    ...
    for i in range(NetNum + 1):
        if i == 0:
            ...
            g = nx.read_adjlist(pathNet + NetName +  '_' + str(i - 1), nodetype=int)
        S = nx.to_scipy_sparse_array(g)
        _, K = autoencoder.predict(S.toarray())
        np.savetxt(path_Surrogate + 'Emb_' + NetName + '_' + str(i), K)
        N = g.number_of_nodes()
